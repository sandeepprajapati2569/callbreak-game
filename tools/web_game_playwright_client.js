...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    if (arg === "--url" && next) {
      ...
      const [width, height] = next.split("x").map((value) => parseInt(value, 10));
...
async function doChoreography(page, canvas, steps) {
  for (const step of steps) {
    ...
    for (const button of buttons) {
      if (button === "left_mouse_button" || button === "right_mouse_button") {
        ...
        const x = typeof step.mouse_x === "number"
          ? step.mouse_x
          : (bbox ? bbox.width / 2 : (viewport ? viewport.width / 2 : 0));
        const y = typeof step.mouse_y === "number"
          ? step.mouse_y
          : (bbox ? bbox.height / 2 : (viewport ? viewport.height / 2 : 0));
        await page.mouse.move(bbox ? bbox.x + x : x, bbox ? bbox.y + y : y);
    ...
    const frames = Number.isFinite(step.frames) ? step.frames : 1;
...
async function main() {
  ...
  const page = await browser.newPage(
    args.viewport ? { viewport: args.viewport } : undefined
  );
